...
- Download and configure mediasoup-worker (v3.19.18 or higher is required):

```bash
$ mkdir -p worker-bin && cd worker-bin
$ npm init -y
$ npm install mediasoup@3.19.19
$ cp node_modules/mediasoup/worker/out/Release/mediasoup-worker ../mediasoup-worker
$ cd .. && rm -rf worker-bin
```

Alternatively, you can download the pre-built `mediasoup-worker` binary from [mediasoup releases](https://github.com/versatica/mediasoup/releases) and place it in your project.

- Run the golang server application in a terminal:

```bash
$ cd server
$ MEDIASOUP_WORKER_BIN="../mediasoup-worker" ./server
```
